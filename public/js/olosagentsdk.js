var OlosAgent = (function (require$$1) {
	...
	const OlosAgent = (function () {
		...
		function callGetCampaignId(logincampaign) {
			$.ajax({
				url: '/OlosAgentAuthenticated.aspx/LoginCampaign',
				type: 'POST',
				contentType: 'application/json; charset=utf-8',
		...
		function GetReceptivaCampaignId(screenPop) {
			$.ajax({
				...
				dataType: 'json',
				success: function (response) {
					campaignIdReceptivaGlobal = response.d;
					console.log('CampaignId: ', campaignIdReceptivaGlobal);
					$('#campaignIdReceptiva').text('CampaignId: ' + campaignIdReceptivaGlobal);
				},
				error: function (error) {
					console.error('Erro ao processar evento no servidor:', error);
				}
